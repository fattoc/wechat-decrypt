...
def _transcribe_local_id(username, local_id, backend):
    row = mcp._fetch_voice_row(username, local_id)
    if row is None:
        return "[not found]"

    voice_data, create_time = row
    try:
        wav_path, _ = mcp._silk_to_wav(voice_data, create_time, username, local_id)
    except Exception as e:
        return f"[decode error: {e}]"

    try:
        result = mcp._transcribe(wav_path, backend)
        return result["text"]
    except Exception as e:
...
def transcribe_export(input_path, output_path):
    ...
    if not username:
        username = mcp.resolve_username(chat_name)
    if not username:
